...
class Session(models.Model):
    ...
    attendee_ids = fields.Many2many('res.partner', string="Attendees" )
                #in attendee_ids states={'done': [('readonly', True)], 'cancel': [('readonly', True)]})
    ...
    state = fields.Selection(
        [('prep','Preparation'),('ready','Ready'),('confirmed','Confirmed')],
        string='State', default = 'prep', track_visibility = 'onchange', readonly=False)
    ...
    def _taken_seats(self):
        for session in self:
            if not session.seats:
                ...
                if session.taken_seats >= 50.0 and session.state == 'ready':
