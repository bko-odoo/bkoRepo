...
class Session(models.Model):
    ...
    @api.multi
    def action_session_attendees(self):
        self.ensure_one()
        print len(self.attendee_ids)
        atts = self.mapped(lambda x : x.attendee_ids)
        action = self.env.ref('openacademy.contact_list_action').read()[0]
        action['views'] = [(self.env.ref('base.view_partner_tree').id, 'tree')]
        #action['res_id'] = self.attendee_ids // to send only one record for form view
        action['domain'] = [('id', 'in', self.attendee_ids.ids)] #recordset has ids function to return ids of all records in list format
